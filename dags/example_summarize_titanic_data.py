...
@aql.run_raw_sql(conn_id="duckdb_default", task_id="dead_users", results_format="pandas_dataframe")
def dead_users_func(aggregate_sql: Table):
    return """
    select * from {{aggregate_sql}} where alive == 'yes'
    """

@aql.dataframe(task_id="single_user")
def single_user_func(aggregate_sql: pd.DataFrame):
    import pandas as pd
...
@dag(
    ...
    schedule=None,
    start_date=pendulum.from_format("2024-03-21", "YYYY-MM-DD"),
    catchup=False,
    owner_links={
        "Rory Smyth": "mailto:rory.smyth@databricks.com",
        "Open in Cloud IDE": "https://cloud.astronomer.io/cltzs7j1f01mw01prrs3m6kb0/cloud-ide/cltzs91op01fy01mlueja1yk8/cltzs91wn01n601prhp5s1gsi",
    },
)
def example_summarize_titanic_data():
    ...
    dead_users = dead_users_func(
        aggregate_sql,
    )

    single_user = single_user_func(
        aggregate_sql,
    )

    dead_users << aggregate_sql
